...
- 公开发现: 已发布文档、专栏、创作者、标签和热搜内容会通过 SEO 友好的社区页面呈现。
...
- 全局搜索: 支持私有知识的向量/全文搜索，也支持公开文档、专栏、创作者和标签发现。
- 内置 MCP: 同时具备 MCP Client 与 MCP Server 能力。
- 自动播客: 文档/专栏可自动生成与更新播客音频。
- AI 插图: 可生成并嵌入高质量插图内容。
- 热搜一站式: 集成 DailyHotApi 查看多平台热榜。
- 公开社区: 可浏览公开专栏/文档，按公开标签筛选，访问创作者主页，并查看公开热搜页。
- Markdown 阅读与编辑增强: 优化表格、Mermaid、图片、内容外壳，以及长文档浮动目录。
- 多语言与响应式: 支持多语言，适配移动端与桌面端。
- 分层防护: 网关前置反爬，API 侧补充限流，对公开高风险接口做双层保护。
