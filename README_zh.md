...
- **音频转写** —— 把音频文档转写成可检索的 Markdown，并提供说话人分离的会议模式。
...
- **通知系统** —— 多渠道（邮件、站内、推送），按事件推送（处理完成、评论、订阅、协作申请等）。
- **账户安全（MFA）** —— 支持 TOTP 验证器与通行密钥（Passkey），并对删除最后一个登录方式做保护。
...
> [!NOTE]
> `app/`、`api/`、`worker/` 是一个 **uv workspace**：一份 `uv.lock`、仓库根一个共享 `.venv`，在仓库根装一次即可，不用每个服务单独建环境。
...
> **不需要初始化数据库。** API 每次启动都会自己建表、补列、补齐内置数据，
> 并确保 Milvus 集合存在 —— 全新安装和版本升级都只要把服务起起来。
> 写在文档里的手动步骤，就是迟早会有人漏掉的步骤。
...
**这一步不能跳过。** 这里曾经写着「可选」，那是错的：`web/.env.example` 里
`NEXT_PUBLIC_API_PREFIX` 指向的是 `http://localhost:8787/api` —— 也就是网关，
不是 api 的 8001。照着「可选」跳过它，前端所有请求都打在一个没人监听的端口上，
界面满屏 Load failed，而 api 日志里干干净净一条请求都没有，看不出任何异常。

网关负责公网流量入口、上游切换，以及在流量进入 `api/` 前先做一层反爬与限流。
热搜也走它转发（`8787/hot` → hot-news 的 6688）。
