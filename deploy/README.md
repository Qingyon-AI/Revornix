...
**线上一共三台机器**，下面三节各讲一台。这里的 unit 来自应用主机；境外那台
...
这台只跑 nginx + `revornix-api`，没有 worker / web / gateway，规格也比应用主机小得多。
它**不单独建库**，用的是应用主机那一套 —— 也就是说它的 api 启动时同样会跑一遍
建表 / 补列 / 种子。`schema_guard` 只加不删，所以两台跑不同版本的代码是安全的，
但要知道有这回事。

**这台长期不在任何文档里。** 它只通过域名被引用，按 IP 在另外两台上搜不到任何结果
（实测），所以前两轮梳理拓扑时都漏了它 —— 配置里的一个域名是一条指向未知主机的边，
而它看起来只是个字符串。部署前先确认自己手上的机器清单是完整的。

> 各实例的地址、私钥文件名、以及某台机器当前缺了哪个依赖，都**不写进这个仓库** ——
> 这是公开仓库，那些东西合在一起就是一份现成的攻击地图，而且对读代码的人毫无用处。
> 它们属于你自己的运维记录。
...
部署机上那把叫 `<GitHub 私钥文件>`，没有 config 时 ssh 根本不会用它，
`git fetch` 报的是 `Permission denied (publickey)` —— 看着像 key 没权限，
实际是 key 压根没被读到：

```
Host github.com
  IdentityFile ~/.ssh/<GitHub 私钥文件>
  IdentitiesOnly yes
```
