...
- `worker`: the async workflow and heavy-job execution layer
...
In practice, `gateway` owns the public entrypoint and path-level routing, `web` handles interaction and presentation, `api` handles state changes and access control, `worker` handles long-running processing, and `hot-news` handles trend aggregation.
...
flowchart LR
    ...
    C --> D["worker runs heavy workflows<br/>It handles document conversion, section generation, podcast, graph, PPT, and notifications"]
...
The private workspace also includes a command-panel/global-search path. It can search the current user's documents through either vector mode or text mode, and the API fuses semantic chunk results with title matches so exact-title hits are not buried by semantic-only candidates.
...
3. `api` creates a document processing task and hands heavy work to `worker`
4. `worker` runs website parsing, file parsing, transcription, Markdown conversion, summarization, vectorization, graph generation, and related workflows based on the document type
...
4. `worker` aggregates section documents, generates Markdown, adds illustrations, generates podcasts, builds section graphs, and creates PPT output when needed
...
4. `api` or `worker` assembles the content and delivers it to email, Feishu, DingTalk, Telegram, or Apple devices
...
The public layer now also supports:

- `/community` tabs for public sections and public documents
- Public label filters for both sections and documents
- Public creator search through the public user search endpoint
- Public document search with vector or text mode
- A floating table of contents for long public Markdown documents
...
The private dashboard route is `/dashboard/hot-search`. There is also a public SEO route at `/hot-search`, which reuses the same normalized hot-list cards for unauthenticated browsing.
