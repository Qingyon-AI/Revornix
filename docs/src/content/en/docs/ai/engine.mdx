...
- `segments`: the engine can return per-segment start time, end time, and text.
- `diarization`: the engine can return speaker labels. If an engine supports `segments` but not `diarization`, Revornix can still generate timestamped segments, but speaker separation degrades.
