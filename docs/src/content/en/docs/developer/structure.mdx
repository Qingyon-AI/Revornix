...
```text
Revornix/
├── app/             # The application itself: models, crud, schemas, engine, notification, data, common
├── api/             # FastAPI entry point (router / mcp_router, thin)
├── worker/          # Celery entry point (workflow, thin)
├── gateway/         # Unified public-entry gateway (Go)
...
├── deploy/          # Production systemd units and deployment notes
├── assets/          # Shared static assets
├── volumes/         # Local persistence for docker-compose-local
├── pyproject.toml   # uv workspace root; members are app / api / worker
├── uv.lock          # One lockfile shared by all three members
...
- Runs from `app/common/celery/app.py` (`worker/` holds only `workflow/`).
...
3. `worker` performs parsing, chunking, embedding, graph generation, illustration or podcast workflows, and writes results back.
...
3. Start `api`, `worker`, `hot-news`, and `web`

## 7. Important implementation boundaries

### AI and official hosted resources

- The official hosted model and engine seed logic lives in `app/data/sql/bootstrap.py`.
...
- There is a single file-system abstraction in `app/file/*`, shared by api and worker.
