...
```mermaid
flowchart LR
    A["External system sends a request<br/>The caller uses an API-KEY with the /tp endpoint group"] --> B["Upload or create content<br/>Upload a file first or create documents and sections directly"]
    B --> C["API stores state and tasks<br/>The service persists state and queues async follow-up work"]
    C --> D["Worker runs workflows<br/>Document and section processing continue in the background"]
    D --> E["Poll for results<br/>The caller uses detail, search, or status endpoints to fetch outputs"]
```
...
The product API also exposes a public section-label list for the community pages. It returns labels that are actually attached to published sections, ordered by usage, and is used by the public `/community` label filter.
...
| `auto_transcribe` | boolean | No | Whether to automatically transcribe an audio document |
| `auto_tag` | boolean | No | Whether to auto-tag |
| `audio_meeting_mode` | boolean | No | Whether an audio document should use meeting-record mode; requires a transcription engine with segment support |
...
To create a meeting-record audio document, upload the audio file to the default file system first, then create a `category=3` audio document with transcription and meeting-record mode enabled:
...
After meeting-record transcription succeeds, the document detail contains the regular Markdown transcript plus a structured segment JSON artifact. The segment JSON powers the fullscreen-player Transcript panel, timestamp seeking, speaker renaming, and right-sidebar meeting insights.
...
The app-level vector search request now supports a `mode` field:

| Field | Value | Description |
| --- | --- | --- |
| `mode` | `vector` | Hybrid semantic retrieval with title-match fusion |
| `mode` | `text` | Keyword/title-description search without vector retrieval |

Vector responses can include a `snippets` map keyed by document ID. Those snippets come from the recalled chunks and are useful for search previews.

Public document discovery uses the same idea through the app-level public vector-search endpoint. It post-filters vector candidates to published documents only, so private documents are not leaked into public search results.
