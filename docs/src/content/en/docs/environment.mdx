...
- Application services are started manually: `gateway`, `api`, `worker`, `web`, `hot-news`
...
| Celery Worker | `worker/.env.example` | `worker/.env` | Multiple `load_dotenv(override=True)` calls | Same behavior as API |
...
- Many `api` and `worker` entrypoints call `load_dotenv(override=True)`.
...
- `app/common/celery/app.py`
- `app/common/dependencies.py`
- `app/encryption.py`
- `app/proxy/notification_proxy.py`
...
| --- | --- | --- | --- |
| `ROOT_USER_NAME` / `ROOT_USER_PASSWORD` | When running `python -m data.sql.create` | `app/data/sql/bootstrap.py` | Creates the root user |
| `OFFICIAL_MODEL_PROVIDER_API_KEY` / `OFFICIAL_MODEL_PROVIDER_BASE_URL` | When `OFFICIAL` is enabled and the official seed path is executed | `app/data/sql/bootstrap.py` | Initializes the official model provider |
...
| --- | --- | --- | --- | --- | --- |
| `OAUTH_SECRET_KEY` | api, worker | Required at startup | None | `app/config/oauth2.py`, `app/common/jwt_utils.py`, `app/config/oauth2.py` | JWT signing key; API and Worker must use the same value |
| `POSTGRES_USER` / `POSTGRES_PASSWORD` / `POSTGRES_DB_URL` / `POSTGRES_DB` | api, worker | Required at startup | Example: `revornix` / `12345678` / `localhost` / `revornix` | `app/data/sql/base.py`, `app/data/sql/bootstrap.py` | PostgreSQL connection |
| `REDIS_URL` / `REDIS_PORT` | api, worker | Required at startup | Example: `localhost` / `6379` | `app/common/redis.py`, `app/common/celery/app.py`, `app/common/celery/app.py` | API cache and Celery broker/backend |
| `MILVUS_CLUSTER_ENDPOINT` / `MILVUS_TOKEN` | api, worker | Required at startup | Example: `http://localhost:19530` / `root:Milvus` | `app/data/milvus/base.py`, `app/data/milvus/base.py` | Vector database |
| `NEO4J_USER` / `NEO4J_PASS` / `NEO4J_URI` | api, worker | Required at startup | Example: `neo4j` / `12345678` / `bolt://localhost:7687` | `app/data/neo4j/base.py`, `app/data/neo4j/base.py` | Graph database |
| `FILE_SYSTEM_SERVER_PUBLIC_URL` / `FILE_SYSTEM_USER_NAME` / `FILE_SYSTEM_PASSWORD` | api, worker | Required at startup | Example: local MinIO | `app/file/built_in_remote_file_service.py`, `app/file/built_in_remote_file_service.py` | Built-in file system (MinIO / S3-compatible) |
| `APIKEY_ENCRYPT_KEY` | api, worker | Required at startup | None | `app/encryption.py`, `app/encryption.py` | Master key for API key encryption |
...
| `LANGFUSE_PUBLIC_KEY` / `LANGFUSE_SECRET_KEY` | api, worker | Required at startup | None | `app/common/dependencies.py`, `app/common/dependencies.py` | The current code validates them strictly during import |
| `LANGFUSE_BASE_URL` | api, worker | Optional | None | `app/config/langfuse.py`, `app/config/langfuse.py` | Currently only read into config; there is no direct call site in this repo yet |
| `OFFICIAL` | api, worker | Optional | Example: `False` | `app/common/dependencies.py`, `app/common/dependencies.py` | Official deployment switch; uses case-insensitive boolean parsing |
| `DEPLOY_HOSTS` | api (worker currently keeps config only) | Optional | Config default is `localhost`; the sample shows official domains | `app/config/base.py`, `app/common/dependencies.py`, `app/config/base.py` | Official-host detection on the API side; Worker does not currently consume it further |
| `WEB_BASE_URL` | api, worker | Recommended | None | `api/notification/tool/*.py`, `worker/notification/tool/*.py`, `app/common/oauth_redirect.py` | Expands relative links into full notification links and acts as the unified public callback base for Google / GitHub OAuth |
...
| `WECHAT_HTTP_TIMEOUT_SECONDS` | api | Optional | `20` | `app/common/tp_auth/wechat_utils.py` | Timeout in seconds for WeChat HTTP requests |
| `SMTP_HOST` / `SMTP_PORT` / `SMTP_USERNAME` / `SMTP_PASSWORD` | api | Currently required for API startup | None | `app/common/system_email/email.py`, `api/router/user.py`, `api/router/notification_target_manage.py` | Email verification codes and notification email |
| `TENCENT_SECRET_ID` / `TENCENT_SECRET_KEY` / `TENCENT_SMS_SDK_APP_ID` / `TENCENT_SMS_APP_KEY` / `TENCENT_SMS_SIGN` | api | Currently required for API startup | None | `app/common/sms/tencent_sms.py`, `api/router/user_auth_phone.py` | Phone verification codes |
| `IOS_DEEPLINK_SCHEME` | api, worker | Optional | `revornix` | `app/notification/tool/apple.py`, `app/notification/tool/apple.py` | Deep link scheme for iOS push notifications |

### 4.3 Bilibili / YouTube credentials and startup authentication

| Variable | Services | Required level | Default / example | Main read points | Purpose |
| --- | --- | --- | --- | --- | --- |
| `REVORNIX_CREDENTIAL_DIR` | api, worker | Optional | `~/.local/state/revornix` | `app/engine/video_plugins/bilibili_auth.py`, `app/engine/video_plugins/youtube_auth.py` | Root directory for stored credentials |
...
| `WORKER_SENTRY_ENABLE` / `WORKER_SENTRY_DSN` | worker | Optional | Disabled by default | `app/common/celery/app.py` | Worker Sentry initialization with `CeleryIntegration` |
| `WORKFLOW_TIMING_VERBOSE` | worker | Optional | `0` | `app/common/timing.py` | Emit detailed workflow timing logs |
| `ALI_DASHSCOPE_EMBEDDING_ON` | api, worker | Optional | `False` | `app/engine/embedding/factory.py`, `app/engine/embedding/factory.py` | Switch to Alibaba DashScope embedding |
| `ALI_DASHSCOPE_EMBEDDING_API_KEY` | api, worker | Required when the previous switch is true | None | `app/engine/embedding/qwen_cloud.py`, same file name in worker | DashScope embedding API key |
| `WS_OFFLINE_CACHE_TTL_SECONDS` | api | Optional | `86400` | `app/common/websocket.py` | TTL for cached offline WebSocket messages |
| `WS_OFFLINE_CACHE_MAX_MESSAGES` | api | Optional | `200` | `app/common/websocket.py` | Max number of retained offline WebSocket messages |

### 4.5 Official deployment and seed-only variables

| Variable | Services | Required level | Default / example | Main read points | Purpose |
| --- | --- | --- | --- | --- | --- |
| `ROOT_USER_NAME` / `ROOT_USER_PASSWORD` | api | Required by initialization scripts | None | `app/data/sql/bootstrap.py` | Creates the root user and default resources |
| `OFFICIAL_MODEL_PROVIDER_API_KEY` / `OFFICIAL_MODEL_PROVIDER_BASE_URL` | api | Required when `OFFICIAL=true` and seeding official models | None | `app/data/sql/bootstrap.py` | Initializes the official model provider |

Additional note:

- These four variables also appear in `worker/.env.example`, but the current normal Worker startup path does not use them.
...
### 4.7 Document upload limits (per subscription tier)

The per-file upload size cap for documents (the `files/` prefix) varies by the user's subscription tier and is **configured only on the API side** via the environment variables below (in MB), falling back to defaults when unset or invalid. The API enforces the limit (including the `content-length-range` on presigned URLs). The Web frontend no longer hardcodes any values — it reads the current user's limit and upgrade eligibility at runtime via the `POST /file-system/upload-limits` endpoint, so the frontend never drifts from the backend config. Admin / Root users and deployments without the payment system are handled accordingly (falling back to the free tier by default, with admins treated as Max).

| Variable | Service | Required level | Default | Main read points | Purpose |
| --- | --- | --- | --- | --- | --- |
| `FILE_DOCUMENT_MAX_UPLOAD_MB_FREE` | api | Optional | `10` | `app/config/upload.py` | Free-tier per-file upload limit (MB) |
| `FILE_DOCUMENT_MAX_UPLOAD_MB_PRO` | api | Optional | `50` | Same as above | Pro-tier per-file upload limit (MB) |
| `FILE_DOCUMENT_MAX_UPLOAD_MB_MAX` | api | Optional | `100` | Same as above | Max-tier per-file upload limit (MB) |

> Aliyun OSS uses a presigned PUT URL, which cannot carry a size policy, so its tier-based limit is only backstopped by the server-mediated upload check.

### 4.8 Gateway variables
...
These are mostly there so you can copy from one aggregated local sample into `api/.env`, `worker/.env`, and `web/.env` when configuring your machine:
