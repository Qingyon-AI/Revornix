...
| `GATEWAY_API_UPSTREAMS` | Recommended | `http://127.0.0.1:8001` | Same as above | Upstream pool for the domestic main `api`; supports multiple addresses |
| `GATEWAY_AUTH_API_UPSTREAMS` | Required when split-region Google / GitHub login is enabled | `http://127.0.0.1:8001` | Same as above | Upstream pool for the overseas auth `api` |
| `GATEWAY_HOT_NEWS_UPSTREAMS` | Recommended | `http://127.0.0.1:6688` | Same as above | Upstream pool for `hot-news` |
