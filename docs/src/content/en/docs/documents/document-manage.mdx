...
- Audio documents go through transcription first, then can continue into summary, graph, and podcast-related workflows. When meeting-record mode is enabled, the transcript also includes speaker separation and clickable timestamps.
...
- Auto summary
- Auto tagging
- Auto transcription for audio documents
- Meeting-record mode for audio documents
- Auto podcast
...
### Audio transcription and meeting-record mode

After transcription succeeds, an audio document stores its transcript as Markdown so it can continue into summary, graph, embedding, and podcast workflows. Regular transcription is best for single-speaker audio or sources where speaker separation is not important.

When you enable meeting-record mode while creating an audio document, Revornix asks a transcription engine with segment support to produce a meeting-style transcript:

- Each segment includes a speaker label such as `S1` or `S2`.
- Each segment includes a timestamp; newly generated transcripts display millisecond precision, such as `00:07.860`.
- Clicking a timestamp in the Markdown body jumps the source audio to that point.
- The fullscreen audio player's Transcript panel shows the same segmented text and can also seek by segment.
- The right sidebar shows meeting insights, including overview, action items, and decisions.
- Speaker names can be renamed from the detail page, for example changing `S1` to a real participant name. Renaming affects the display name only; the original transcript artifact is not rewritten.

<Callout type='info'>
	Meeting-record mode depends on the transcription engine's segment capability. If the default audio transcription engine cannot return segments or speaker labels, the create page disables meeting-record mode or asks you to switch to a capable engine.
</Callout>
...
Documents can be shared with collaborators using the same model as sections: identity expresses only the relationship (creator / collaborator), permission expresses only the collaboration capability (FULL_ACCESS / READ_AND_WRITE / READ_ONLY). Capabilities are split as follows:

| Action | Creator | FULL_ACCESS | READ_AND_WRITE | READ_ONLY |
| --- | :---: | :---: | :---: | :---: |
| Read, comment, note, star, AI Q&A | ✓ | ✓ | ✓ | ✓ |
| Edit Markdown body | ✓ | ✓ | ✓ | ✗ |
| Edit document metadata (title / description / cover / labels) | ✓ | ✓ | ✓ | ✗ |
| Trigger tasks (AI summary, embedding, transcription, podcast, graph, Markdown conversion) | ✓ | ✓ | ✓ | ✗ |
| Invite new collaborators / approve access requests | ✓ | ✓ | ✗ | ✗ |
| Modify or remove collaborators | All collaborators | Only collaborators they invited / approved | ✗ | ✗ |
| Change which sections the document belongs to | ✓ | ✗ | ✗ | ✗ |
| Publish / unpublish | ✓ | ✗ | ✗ | ✗ |
| Delete the document | ✓ | ✗ | ✗ | ✗ |

<Callout type='info'>FULL_ACCESS can bring people in, but can only manage the collaborators it personally brought in. Changing section membership, publishing, and deletion stay creator-only. The rule is enforced at the API layer and by hiding controls in the UI.</Callout>
...
| Audio transcript Markdown / meeting segment JSON | [Custom File Service](../integrations/custom-file-system) |
