...
- 应用服务使用手动方式启动：`gateway`、`api`、`worker`、`web`、`hot-news`
...
| Celery Worker | `worker/.env.example` | `worker/.env` | 多处 `load_dotenv(override=True)` | 行为和 API 一致 |
...
- `api` 和 `worker` 的很多入口都调用了 `load_dotenv(override=True)`。
...
| 变量 | 何时必填 | 主要读取点 | 说明 |
| --- | --- | --- | --- |
| `ROOT_USER_NAME` / `ROOT_USER_PASSWORD` | 运行 `python -m data.sql.create` 时 | `app/data/sql/bootstrap.py` | 创建 Root 用户 |
| `OFFICIAL_MODEL_PROVIDER_API_KEY` / `OFFICIAL_MODEL_PROVIDER_BASE_URL` | `OFFICIAL` 开启且执行官方 seed 路径时 | `app/data/sql/bootstrap.py` | 初始化官方模型提供商 |
...
| --- | --- | --- | --- | --- | --- |
| `OAUTH_SECRET_KEY` | api, worker | 启动必填 | 无 | `app/config/oauth2.py`, `app/common/jwt_utils.py`, `app/config/oauth2.py` | JWT 签名密钥；API 和 Worker 必须一致 |
| `POSTGRES_USER` / `POSTGRES_PASSWORD` / `POSTGRES_DB_URL` / `POSTGRES_DB` | api, worker | 启动必填 | 样例：`revornix` / `12345678` / `localhost` / `revornix` | `app/data/sql/base.py`, `app/data/sql/bootstrap.py` | PostgreSQL 连接 |
| `REDIS_URL` / `REDIS_PORT` | api, worker | 启动必填 | 样例：`localhost` / `6379` | `app/common/redis.py`, `app/common/celery/app.py`, `app/common/celery/app.py` | API 缓存与 Celery broker/backend |
| `MILVUS_CLUSTER_ENDPOINT` / `MILVUS_TOKEN` | api, worker | 启动必填 | 样例：`http://localhost:19530` / `root:Milvus` | `app/data/milvus/base.py`, `app/data/milvus/base.py` | 向量库 |
| `NEO4J_USER` / `NEO4J_PASS` / `NEO4J_URI` | api, worker | 启动必填 | 样例：`neo4j` / `12345678` / `bolt://localhost:7687` | `app/data/neo4j/base.py`, `app/data/neo4j/base.py` | 图数据库 |
| `FILE_SYSTEM_SERVER_PUBLIC_URL` / `FILE_SYSTEM_USER_NAME` / `FILE_SYSTEM_PASSWORD` | api, worker | 启动必填 | 样例：本地 MinIO | `app/file/built_in_remote_file_service.py`, `app/file/built_in_remote_file_service.py` | 内置文件系统（MinIO/S3 兼容） |
| `APIKEY_ENCRYPT_KEY` | api, worker | 启动必填 | 无 | `app/encryption.py`, `app/encryption.py` | API Key 加密主密钥 |
...
| `LANGFUSE_PUBLIC_KEY` / `LANGFUSE_SECRET_KEY` | api, worker | 启动必填 | 无 | `app/common/dependencies.py`, `app/common/dependencies.py` | 当前代码在导入阶段就强校验 |
| `LANGFUSE_BASE_URL` | api, worker | 可选 | 无 | `app/config/langfuse.py`, `app/config/langfuse.py` | 当前代码只读入 config，仓库里暂无直接调用 |
| `OFFICIAL` | api, worker | 可选 | 样例：`False` | `app/common/dependencies.py`, `app/common/dependencies.py` | 官方部署开关；走大小写无关布尔解析 |
| `DEPLOY_HOSTS` | api（worker 当前仅保留配置项） | 可选 | config 默认 `localhost`，样例为官方域名列表 | `app/config/base.py`, `app/common/dependencies.py`, `app/config/base.py` | API 侧官方 host 判断；Worker 当前没有继续消费它 |
| `WEB_BASE_URL` | api, worker | 推荐配置 | 无 | `api/notification/tool/*.py`, `worker/notification/tool/*.py`, `app/common/oauth_redirect.py` | 把相对链接补成完整通知链接，并作为 Google / GitHub OAuth 的统一公网回调入口 |
...
| `WECHAT_HTTP_TIMEOUT_SECONDS` | api | 可选 | `20` | `app/common/tp_auth/wechat_utils.py` | 微信 HTTP 请求超时秒数 |
| `SMTP_HOST` / `SMTP_PORT` / `SMTP_USERNAME` / `SMTP_PASSWORD` | api | 当前 API 启动必填 | 无 | `app/common/system_email/email.py`, `api/router/user.py`, `api/router/notification_target_manage.py` | 邮件验证码、通知邮箱 |
| `TENCENT_SECRET_ID` / `TENCENT_SECRET_KEY` / `TENCENT_SMS_SDK_APP_ID` / `TENCENT_SMS_APP_KEY` / `TENCENT_SMS_SIGN` | api | 当前 API 启动必填 | 无 | `app/common/sms/tencent_sms.py`, `api/router/user_auth_phone.py` | 手机验证码 |
| `IOS_DEEPLINK_SCHEME` | api, worker | 可选 | `revornix` | `app/notification/tool/apple.py`, `app/notification/tool/apple.py` | iOS 推送 deep link scheme |

### 4.3 Bilibili / YouTube 凭证与启动鉴权

| 变量 | 服务 | 必填级别 | 默认值 / 样例 | 主要读取点 | 用途 |
| --- | --- | --- | --- | --- | --- |
| `REVORNIX_CREDENTIAL_DIR` | api, worker | 可选 | `~/.local/state/revornix` | `app/engine/video_plugins/bilibili_auth.py`, `app/engine/video_plugins/youtube_auth.py` | 凭证根目录 |
...
| `WORKER_SENTRY_ENABLE` / `WORKER_SENTRY_DSN` | worker | 可选 | 默认关闭 | `app/common/celery/app.py` | Worker Sentry 初始化，已接入 CeleryIntegration |
| `WORKFLOW_TIMING_VERBOSE` | worker | 可选 | `0` | `app/common/timing.py` | 输出详细 workflow timing 日志 |
| `ALI_DASHSCOPE_EMBEDDING_ON` | api, worker | 可选 | `False` | `app/engine/embedding/factory.py`, `app/engine/embedding/factory.py` | 切换到阿里云百炼 embedding |
| `ALI_DASHSCOPE_EMBEDDING_API_KEY` | api, worker | 上一项为真时必填 | 无 | `app/engine/embedding/qwen_cloud.py`, worker 同名文件 | 百炼 embedding API Key |
| `WS_OFFLINE_CACHE_TTL_SECONDS` | api | 可选 | `86400` | `app/common/websocket.py` | WebSocket 离线消息缓存 TTL |
| `WS_OFFLINE_CACHE_MAX_MESSAGES` | api | 可选 | `200` | `app/common/websocket.py` | WebSocket 离线消息保留条数 |

### 4.5 官方部署和 seed 专属变量

| 变量 | 服务 | 必填级别 | 默认值 / 样例 | 主要读取点 | 用途 |
| --- | --- | --- | --- | --- | --- |
| `ROOT_USER_NAME` / `ROOT_USER_PASSWORD` | api | 初始化脚本必填 | 无 | `app/data/sql/bootstrap.py` | 创建 Root 用户与默认资源 |
| `OFFICIAL_MODEL_PROVIDER_API_KEY` / `OFFICIAL_MODEL_PROVIDER_BASE_URL` | api | `OFFICIAL=true` 且 seed 官方模型时必填 | 无 | `app/data/sql/bootstrap.py` | 初始化官方模型提供商 |

补充：

- 这 4 个变量在样例里同时出现在 `worker/.env.example`，但当前 Worker 正常启动链路并不会用到它们。
...
文档（`files/` 前缀）单文件上传大小上限按用户订阅等级区分，**仅在 API 侧通过下列环境变量配置（单位 MB）**，缺省或非法时回退到默认值。API 侧为强制校验（含预签名 URL 的 `content-length-range`）。Web 前端不再内置任何数值，而是在运行时通过接口 `POST /file-system/upload-limits` 读取「当前用户的上限 + 是否可升级」，因此前端与后端配置零漂移。管理员 / Root 以及未接入支付系统时按对应逻辑处理（默认落到免费档，管理员为 Max）。

| 变量 | 服务 | 必填级别 | 默认值 | 主要读取点 | 用途 |
| --- | --- | --- | --- | --- | --- |
| `FILE_DOCUMENT_MAX_UPLOAD_MB_FREE` | api | 可选 | `10` | `app/config/upload.py` | 免费档单文件上传上限（MB） |
| `FILE_DOCUMENT_MAX_UPLOAD_MB_PRO` | api | 可选 | `50` | 同上 | Pro 档单文件上传上限（MB） |
| `FILE_DOCUMENT_MAX_UPLOAD_MB_MAX` | api | 可选 | `100` | 同上 | Max 档单文件上传上限（MB） |

> 阿里云 OSS 走 PUT 预签名，无法携带大小策略，其分级限额仅由服务端直传校验兜底。
...
这些变量主要是为了让你本地只看一个聚合样例，也能顺手给 `api/.env`、`worker/.env`、`web/.env` 抄配置：
