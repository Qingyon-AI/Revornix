...
| `GATEWAY_API_UPSTREAMS` | 推荐必填 | `http://127.0.0.1:8001` | 同上 | 国内主 `api` 上游池，支持多地址 |
| `GATEWAY_AUTH_API_UPSTREAMS` | 启用 Google / GitHub 跨地域登录时必填 | `http://127.0.0.1:8001` | 同上 | 境外认证 `api` 上游池 |
| `GATEWAY_HOT_NEWS_UPSTREAMS` | 推荐配置 | `http://127.0.0.1:6688` | 同上 | `hot-news` 上游池 |
