...
| --- | --- | --- | --- | --- | --- |
| `OAUTH_SECRET_KEY` | api, worker | 启动必填 | 无 | `api/config/oauth2.py`, `api/common/jwt_utils.py`, `worker/config/oauth2.py` | JWT 签名密钥；API 和 Worker 必须一致 |
| `POSTGRES_USER` / `POSTGRES_PASSWORD` / `POSTGRES_DB_URL` / `POSTGRES_DB` | api, worker | 启动必填 | 样例：`revornix` / `12345678` / `localhost` / `revornix` | `api/data/sql/base.py`, `worker/data/sql/base.py`, `api/alembic/env.py` | PostgreSQL 连接 |
| `REDIS_URL` / `REDIS_PORT` | api, worker | 启动必填 | 样例：`localhost` / `6379` | `api/common/redis.py`, `api/common/celery/app.py`, `worker/common/celery/app.py` | API 缓存与 Celery broker/backend |
| `MILVUS_CLUSTER_ENDPOINT` / `MILVUS_TOKEN` | api, worker | 启动必填 | 样例：`http://localhost:19530` / `root:Milvus` | `api/data/milvus/base.py`, `worker/data/milvus/base.py` | 向量库 |
| `NEO4J_USER` / `NEO4J_PASS` / `NEO4J_URI` | api, worker | 启动必填 | 样例：`neo4j` / `12345678` / `bolt://localhost:7687` | `api/data/neo4j/base.py`, `worker/data/neo4j/base.py` | 图数据库 |
| `FILE_SYSTEM_SERVER_PUBLIC_URL` / `FILE_SYSTEM_USER_NAME` / `FILE_SYSTEM_PASSWORD` | api, worker | 启动必填 | 样例：本地 MinIO | `api/file/built_in_remote_file_service.py`, `worker/file/built_in_remote_file_service.py` | 内置文件系统（MinIO/S3 兼容） |
| `APIKEY_ENCRYPT_KEY` | api, worker | 启动必填 | 无 | `api/common/encrypt.py`, `worker/common/encrypt.py` | API Key 加密主密钥 |
...
| `LANGFUSE_PUBLIC_KEY` / `LANGFUSE_SECRET_KEY` | api, worker | 启动必填 | 无 | `api/common/dependencies.py`, `worker/common/dependencies.py` | 当前代码在导入阶段就强校验 |
| `LANGFUSE_BASE_URL` | api, worker | 可选 | 无 | `api/config/langfuse.py`, `worker/config/langfuse.py` | 当前代码只读入 config，仓库里暂无直接调用 |
| `OFFICIAL` | api, worker | 可选 | 样例：`False` | `api/common/dependencies.py`, `worker/common/dependencies.py` | 官方部署开关；走大小写无关布尔解析 |
| `DEPLOY_HOSTS` | api（worker 当前仅保留配置项） | 可选 | config 默认 `localhost`，样例为官方域名列表 | `api/config/base.py`, `api/common/dependencies.py`, `worker/config/base.py` | API 侧官方 host 判断；Worker 当前没有继续消费它 |
| `WEB_BASE_URL` | api, worker | 推荐配置 | 无 | `api/notification/tool/*.py`, `worker/notification/tool/*.py`, `api/common/oauth_redirect.py` | 把相对链接补成完整通知链接，并作为 Google / GitHub OAuth 的统一公网回调入口 |
...
| `IOS_DEEPLINK_SCHEME` | api, worker | 可选 | `revornix` | `api/notification/tool/apple.py`, `worker/notification/tool/apple.py` | iOS 推送 deep link scheme |
...
| `WORKER_SENTRY_ENABLE` / `WORKER_SENTRY_DSN` | worker | 可选 | 默认关闭 | `worker/common/celery/app.py` | Worker Sentry 初始化，已接入 CeleryIntegration |
| `WORKFLOW_TIMING_VERBOSE` | worker | 可选 | `0` | `worker/workflow/timing.py` | 输出详细 workflow timing 日志 |
| `ALI_DASHSCOPE_EMBEDDING_ON` | api, worker | 可选 | `False` | `api/engine/embedding/factory.py`, `worker/engine/embedding/factory.py` | 切换到阿里云百炼 embedding |
...
| --- | --- | --- | --- | --- | --- |
| `FILE_DOCUMENT_MAX_UPLOAD_MB_FREE` | api | 可选 | `10` | `api/config/upload.py` | 免费档单文件上传上限（MB） |
