...
<ProcessDiagram
	...
	steps={[
		{
			title: '用户访问统一公网域名',
			description: '浏览器始终打开 app.example.com 和 api.example.com。',
		},
		{
			title: '前端发起 Google 或 GitHub 登录',
			description: 'OAuth 回调地址固定注册为统一公网入口，不直接暴露内网或机房地址。',
		},
		{
			title: '第三方平台回调到公网入口',
			description: '回调页仍由统一前端入口承接，浏览器 cookie 和站点上下文保持一致。',
		},
		{
			title: 'API 网关转发 OAuth 接口',
			description: '只有 /user/create/google、/user/create/github、/user/bind/google、/user/bind/github 被转发到境外 API。',
		},
		{
			title: '其余业务继续走国内服务',
			description: '文档、专栏、通知、图谱、PPT、WebSocket 等链路仍由国内主服务承接。',
		},
	]}
/>
