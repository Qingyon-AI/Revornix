...
async def get_usage_ledger_by_idempotency_key_async(
    db: AsyncSession,
    *,
    idempotency_key: str,
):
    stmt = select(models.usage.UsageLedger).where(
        models.usage.UsageLedger.idempotency_key == idempotency_key,
    )
...
async def create_usage_ledger_async(
    db: AsyncSession,
    *,
    user_id: int,
    resource_uuid: str,
...
async def get_monthly_usage_summary_async(
    db: AsyncSession,
    *,
    user_id: int,
    resource_uuid: str,
    ...
    stmt = select(models.usage.MonthlyUsageSummary).where(
        models.usage.MonthlyUsageSummary.user_id == user_id,
        models.usage.MonthlyUsageSummary.resource_uuid == resource_uuid,
        models.usage.MonthlyUsageSummary.cycle_month == cycle_month,
    )
...
async def create_monthly_usage_summary_async(
    db: AsyncSession,
    *,
    user_id: int,
    resource_uuid: str,
...
async def sum_usage_ledger_points_in_window_async(
    db: AsyncSession,
    *,
    user_id: int,
    resource_uuid: str,
