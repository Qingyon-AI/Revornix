...
export const normalizeEditorMarkdown = (markdown: string) => {
	...
	const normalized = normalizeOutsideFencedCodeBlocks(
		withNormalizedCodeBlocks,
		(segment) => segment.replace(/[ \t]+\n/g, '\n'),
	);
	const withTables = normalizeOutsideFencedCodeBlocks(
		normalized,
		convertMarkdownTablesToQuickTables,
	);
	...
	return normalizeOutsideFencedCodeBlocks(withNormalizedBlocks, (segment) =>
		segment.replace(/\n{3,}/g, '\n\n'),
	).trimEnd();
};
