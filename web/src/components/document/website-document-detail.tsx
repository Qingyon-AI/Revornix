...
const WebsiteDocumentDetail = ({
	...
	const handleSaveMarkdown = async (content: string) => {
		if (!activeMarkdownFileName || !userFileSystemDetail?.file_system_id) {
			throw new Error(t('document_markdown_file_missing'));
		}

		const fileName =
			activeMarkdownFileName.split('/').pop() || `document-${id}.md`;
		const fileService = new FileService(userFileSystemDetail.file_system_id);
		const file = new File([content], fileName, {
			type: 'text/markdown;charset=utf-8',
		});

		await fileService.uploadFile(activeMarkdownFileName, file);
