	...
	return (
		<Sheet open={open} onOpenChange={setOpen}>
			<SheetTrigger asChild>
				<Button
					title={t('document_comments')}
					variant={'ghost'}
					className={cn('w-full flex-1 text-xs', className)}
					onClick={onTriggerClick}>
					<MessageCircleMore />
					{iconOnly ? (
						<span className='sr-only'>{t('document_comments')}</span>
					) : (
						t('document_comments')
					)}
				</Button>
			...
			<SheetContent className='flex h-full flex-col gap-0 overflow-hidden bg-card/95 pt-0 sm:max-w-xl'>
				<SheetHeader className='border-b border-border/60 px-5 pt-6 pb-3 pr-12 text-left'>
					<SheetTitle className='text-xl'>{t('document_comments')}</SheetTitle>
...
export default DocumentOperateComment;
