...
import { createLabel } from '@/service/document';
import { z } from 'zod';
...
const AddDocumentLabelDialog = ({
	...
	const formSchema = z.object({
		name: z.string().min(1, t('document_create_label_create_name_needed')),
	});
	...
	const onFormValidateSuccess = async (values: z.infer<typeof formSchema>) => {
		...
		toast.success(t('document_create_label_create_success'));
		onOpenChange(false);
		form.reset();
	};
								...
								return (
									<FormItem>
										<Input
											{...field}
											placeholder={t(
												'document_create_label_create_placeholded'
											)}
										/>
										<FormMessage />
									</FormItem>
						...
						<DialogFooter className='mt-5'>
							...
							<DialogClose asChild>
								<Button type='button' variant={'outline'}>
									{t('document_create_label_create_cancel')}
								</Button>
							</DialogClose>
						</DialogFooter>
					</form>
...
export default AddDocumentLabelDialog;
