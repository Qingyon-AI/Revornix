...
const EntityGraphCanvas = ({
	...
	const [selectedNode, setSelectedNode] = useState<GraphCanvasNode | null>(
		null,
	);
	const [searchQuery, setSearchQuery] = useState('');
	const [activeSearchIndex, setActiveSearchIndex] = useState(0);
	const { resolvedTheme } = useTheme();
	const normalizedSearchQuery = showSearch
	...
	const cycleSearchMatch = (direction: 1 | -1) => {
		...
		setActiveSearchIndex((currentIndex) => {
			...
			return (
				(safeIndex + direction + searchMatches.length) % searchMatches.length
			);
		});
	};
	...
	useEffect(() => {
		...
		const resize = () => {
			...
			const simulationAlphaDecay = isHugeGraph
				? 0.16
				: isLargeGraph
					? 0.1
					: 0.06;
			const simulationAlphaMin = isHugeGraph
				? 0.02
				: isLargeGraph
					? 0.012
					: 0.001;
			const simulationVelocityDecay = isHugeGraph
				? 0.5
				: isLargeGraph
					? 0.42
					: 0.34;
			...
			let labelSelection = labelLayer.selectAll<SVGGElement, SimulationNode>(
				'g.graph-label',
			);
			...
			const getPointerGraphPosition = (
				...
				return {
					graphX,
					graphY,
					isOutside: clampedLocalX !== localX || clampedLocalY !== localY,
				};
			};
			...
			const getFocusedLabelNodes = (
				...
				const focusNeighbors = nodes
					.filter(
						(node) => connectedIds.has(node.id) && node.id !== focusNode.id,
					)
					.sort((left, right) => (right.degree ?? 1) - (left.degree ?? 1))
			...
			const getSearchLabelNodes = () => {
				...
				if (
					...
					if (activeNode != null) {
						return [activeNode, ...rankedMatches].slice(
							0,
							maxFocusedLabelCount,
						);
					}
				}
			...
			const applyHighlight = (focusNode: GraphCanvasNode | null) => {
				if (focusNode == null) {
					if (searchMatchedNodeIds.size > 0) {
						...
						linkElements
							.attr('stroke', (link) => {
								const sourceId = isNode(link.source)
									? link.source.id
									: link.source;
								const targetId = isNode(link.target)
									? link.target.id
									: link.target;
								if (
			...
			const fitGraph = (duration: number) => {
				...
				for (const node of nodes) {
					...
					const labelAllowance = baseVisibleLabelIds.has(node.id)
						? Math.min(
								120,
								truncateLabel(node.label, labelCharLimit).length * 8 + 28,
							)
						: 0;
			...
			const fallbackFitDuration =
				nodeCount > LARGE_GRAPH_NODE_THRESHOLD ? 300 : 360;
			...
			const currentSelectedNode =
				selectedNodeIdRef.current == null
					? null
					: (nodes.find((node) => node.id === selectedNodeIdRef.current) ??
						null);
			searchHighlightRef.current(
				searchMatches.map((node) => node.id),
				activeSearchMatch?.id ?? null,
	...
	return (
		<div
			className={cn(
				'relative h-full min-h-[24rem] overflow-hidden',
