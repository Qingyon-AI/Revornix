...
const SectionInfoSubscriber = ({ section_id }: { section_id: number }) => {
		...
		<div>
			<div className='flex flex-wrap -space-x-1 *:data-[slot=avatar]:ring-background *:data-[slot=avatar]:ring-2 *:data-[slot=avatar]:grayscale'>
				...
				<Sheet>
					...
					<SheetContent>
						<SheetHeader>
							<SheetTitle>{t('section_subscribers')}</SheetTitle>
							<Input
								className='w-full'
						...
						<div className='px-5 flex-1 overflow-auto text-xs text-muted-foreground gap-3 flex flex-col'>
							...
							{isFetching &&
								...
								[...Array(20)].map((number, index) => {
									return <Skeleton className='w-full h-12 mb-3' key={index} />;
								})}
							{/* ✅ 底部加载状态 */}
							{isFetchingNextPage &&
								userPages &&
...
export default SectionInfoSubscriber;
