...
const Nav = async () => {
	...
	const dashboardHref = isLoggedIn
		? '/dashboard'
		: `/login?redirect_to=${encodeURIComponent('/dashboard')}`;

	const navLinks = [
		{ href: '/community', label: t('seo_nav_community') },
		{ href: dashboardHref, label: t('seo_nav_dashboard') },
		...
		<header className='sticky top-0 z-20 border-b border-border/40 bg-background/76 backdrop-blur-xl'>
			<div className='mx-auto flex h-14 w-full max-w-[1480px] items-center justify-between gap-4 px-4 sm:px-6 lg:px-8'>
				<div className='flex min-w-0 items-baseline gap-6'>
					...
					<nav className='hidden items-baseline gap-4 md:flex'>
						{navLinks.map((item) => (
							<Link
								key={item.href}
								href={item.href}
				...
				<div className='flex shrink-0 items-center gap-1.5'>
					<Button
						...
						aria-label={t('seo_nav_dashboard')}>
