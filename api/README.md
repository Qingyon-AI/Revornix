...
- **Changing a table takes two steps.** There is no migration framework at runtime. The schema is `Base.metadata.create_all` (fresh installs, in `data/sql/create.py`) plus a chain of idempotent `_migrate_*` functions in `data/sql/schema_guard.py` (existing installs), which both the API and the celery worker run on startup. So: (1) change `models/*.py` — fresh installs get the right shape from it; (2) add a `_migrate_*` and hook it into `run_schema_guard()` — existing installs catch up from it. Do only the first and a fresh install works while everyone upgrading crashes on the missing column. Upgrades need no manual step; `python -m data.sql.create` stays a fresh-database bootstrap. (Alembic used to live here, but its migrations were never version-controlled — every deployment autogenerated its own chain with its own revision ids, so no migration could be shared and every install had to be migrated by hand.)
