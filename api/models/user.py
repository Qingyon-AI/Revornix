...
class WechatUser(Base):
    ...
    wechat_user_union_id: Mapped[Optional[str]] = mapped_column(String(100))
    wechat_user_open_id: Mapped[Optional[str]] = mapped_column(String(100), index=True)
    wechat_user_name: Mapped[Optional[str]] = mapped_column(String(100))
    wechat_platform: Mapped[Optional[int]] = mapped_column(Integer, comment='0: unknown, 1: Revornix WeChat Web App, 2: Revornix WeChat Mini Program')
...
class GoogleUser(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    user_id: Mapped[Optional[int]] = mapped_column(ForeignKey("user.id"), index=True)
    google_user_id: Mapped[Optional[str]] = mapped_column(String(100), index=True)
    google_user_name: Mapped[Optional[str]] = mapped_column(String(100))
    delete_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True))
...
class GithubUser(Base):
    ...
    github_user_name: Mapped[Optional[str]] = mapped_column(String(100))
    delete_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True))
...
class FollowUser(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    to_user_id: Mapped[Optional[int]] = mapped_column(ForeignKey("user.id"), index=True)
    from_user_id: Mapped[Optional[int]] = mapped_column(ForeignKey("user.id"), index=True)
