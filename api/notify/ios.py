...
class IOSNotify(NotifyProtocol):
    def __init__(self, 
                 source_id: int, 
                 target_id: int):
        super().__init__(notify_uuid='ec2101d2d2134fe398626487eae7b05c',
                         notify_name='IOSNotify',
                         notify_name_zh='iOS通知',
                         notify_description='Send notification via ios apns',
                         notify_description_zh='通过iOS APNS发送通知',
                         source_id=source_id, 
                         target_id=target_id)
    ...
    def _fetch_apple_public_keys(self):
        """
    ...
    def _get_public_key(self, kid, keys):
        """
    ...
    def _convert_jwk_to_pem(self, jwk_data):
        """
    ...
    def _verify_jwt(self, identity_token, public_key, audience: str):
        """
        验证 JWT 签名并解码。
        """
        decoded = jwt.decode(
    ...
    def _decode_identity_token(self, identity_token: str):
        """
        ...
        # Step 5: 验证 JWT
        res = self._verify_jwt(identity_token, pem_key)
        return res
