...
@graph_router.post('/section/seo', response_model=schemas.graph.GraphResponse)
async def seo_graph(seo_section_graph_request: schemas.graph.SectionGraphRequest,
                    db: Session = Depends(get_db)):
    section_id = seo_section_graph_request.section_id
...
async def section_graph(section_graph_request: schemas.graph.SectionGraphRequest,
    ...
    if not section.public and user.id not in [user.id for user in section_user]:
        raise Exception("You are not authorized to view this section")
