...
@notification_router.post('/target/email/send', response_model=schemas.common.NormalResponse)
...
@notification_router.post('/target/ios/code/status', response_model=schemas.common.NormalResponse)
...
async def add_notification_target(
    ...
    if db_notification_target_provided.uuid == NotificationTargetProvided.EMAIL.meta.uuid:
        ...
        target_email = add_notification_target_request.email_target_form.email
        if not target_email:
            raise schemas.error.CustomException(message="email should not be empty", code=400)
        cached_code = await cache.get(
            name=f'{user.id}-user-notification-target-add-{target_email}',
        )
        code = add_notification_target_request.email_target_form.code
        if cached_code != code:
    ...
    if db_notification_target_provided.uuid == NotificationTargetProvided.APPLE.meta.uuid or db_notification_target_provided.uuid == NotificationTargetProvided.APPLE_SANDBOX.meta.uuid:
        if add_notification_target_request.ios_target_form is None:
            raise schemas.error.CustomException(message="ios config should not be empty", code=400)
        if add_notification_target_request.ios_target_form.device_token is None:
            raise schemas.error.CustomException(message="device_token should not be empty", code=400)
        config_json = json.dumps({
            "device_token": add_notification_target_request.ios_target_form.device_token
        })
    
    if db_notification_target_provided.uuid == NotificationTargetProvided.FEISHU.meta.uuid:
        if add_notification_target_request.feishu_target_form is None:
            raise schemas.error.CustomException(message="feishu config should not be empty", code=400)
        config_json = json.dumps({
            "webhook_url": add_notification_target_request.feishu_target_form.webhook_url,
            "sign": add_notification_target_request.feishu_target_form.sign
        })

    if db_notification_target_provided.uuid == NotificationTargetProvided.DINGTALK.meta.uuid:
        if add_notification_target_request.dingtalk_target_form is None:
            raise schemas.error.CustomException(message="dingtalk config should not be empty", code=400)
        config_json = json.dumps({
            "webhook_url": add_notification_target_request.dingtalk_target_form.webhook_url,
            "sign": add_notification_target_request.dingtalk_target_form.sign
        })
    
    if db_notification_target_provided.uuid == NotificationTargetProvided.TELEGRAM.meta.uuid:
        if add_notification_target_request.telegram_target_form is None:
            raise schemas.error.CustomException(message="telegram config should not be empty", code=400)
        config_json = json.dumps({
            "chat_id": add_notification_target_request.telegram_target_form.chat_id,
        })
...
async def update_notification_target(
    ...
    config_json = None
    # 邮件类型通知终端
    if db_notification_target.notification_target_provided.uuid == NotificationTargetProvided.EMAIL.meta.uuid:
        if update_notification_target_request.email_target_form is not None:
            target_email = update_notification_target_request.email_target_form.email
            if not target_email:
                raise schemas.error.CustomException(message="email should not be empty", code=400)
            cached_code = await cache.get(
                name=f'{user.id}-user-notification-target-add-{target_email}',
            )
            code = update_notification_target_request.email_target_form.code
            if cached_code != code:
                raise schemas.error.CustomException(message="The code is incorrect", code=400)
            cache.delete(
                f'{user.id}-user-notification-target-add-{target_email}',
    ...
    if db_notification_target.notification_target_provided.uuid == NotificationTargetProvided.APPLE.meta.uuid or db_notification_target.notification_target_provided.uuid == NotificationTargetProvided.APPLE_SANDBOX.meta.uuid:
        if update_notification_target_request.ios_target_form is not None:
            if not update_notification_target_request.ios_target_form.device_token:
                raise schemas.error.CustomException(message="device_token should not be empty", code=400)
            config_json = json.dumps({
                "device_token": update_notification_target_request.ios_target_form.device_token
            })
    
    if db_notification_target.notification_target_provided.uuid == NotificationTargetProvided.FEISHU.meta.uuid:
        if update_notification_target_request.feishu_target_form is not None:
            if not update_notification_target_request.feishu_target_form.webhook_url or not update_notification_target_request.feishu_target_form.sign: 
                raise schemas.error.CustomException(message="webhook_url or sign should not be empty", code=400)
            config_json = json.dumps({
                "webhook_url": update_notification_target_request.feishu_target_form.webhook_url,
                "secret": update_notification_target_request.feishu_target_form.sign
            })

    if db_notification_target.notification_target_provided.uuid == NotificationTargetProvided.DINGTALK.meta.uuid:
        if update_notification_target_request.dingtalk_target_form is not None:
            if not update_notification_target_request.dingtalk_target_form.webhook_url or not update_notification_target_request.dingtalk_target_form.sign: 
                raise schemas.error.CustomException(message="webhook_url or sign should not be empty", code=400)
            config_json = json.dumps({
                "webhook_url": update_notification_target_request.dingtalk_target_form.webhook_url,
                "sign": update_notification_target_request.dingtalk_target_form.sign
            })
    
    if db_notification_target.notification_target_provided.uuid == NotificationTargetProvided.TELEGRAM.meta.uuid:
        if update_notification_target_request.telegram_target_form is not None:
            if not update_notification_target_request.telegram_target_form.chat_id:
                raise schemas.error.CustomException(message="chat_id should not be empty", code=400)
            config_json = json.dumps({
                "chat_id": update_notification_target_request.telegram_target_form.chat_id,
            })
