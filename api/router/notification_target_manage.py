    ...
    *,
    ...
    if target_provided_uuid == NotificationTargetProvided.EMAIL.meta.uuid:
        if email_target_form is None:
            if require_config:
                raise schemas.error.CustomException(message="Email target configuration is required", code=400)
            return None
        ...
        if cached_code != email_target_form.code:
            raise schemas.error.CustomException(message="Verification code is incorrect", code=400)
        await cache.delete(
    ...
    if target_provided_uuid in {
        ...
        if ios_target_form is None:
            if require_config:
                raise schemas.error.CustomException(message="iOS target configuration is required", code=400)
            return None
        if not ios_target_form.device_token:
            raise schemas.error.CustomException(message="Device token is required", code=400)
        return json.dumps(
            {
    ...
    if target_provided_uuid == NotificationTargetProvided.FEISHU.meta.uuid:
        if feishu_target_form is None:
            if require_config:
                raise schemas.error.CustomException(message="Feishu target configuration is required", code=400)
            return None
        if not feishu_target_form.webhook_url or not feishu_target_form.sign:
            raise schemas.error.CustomException(message="Webhook URL and sign are required", code=400)
        return json.dumps(
            {
    ...
    if target_provided_uuid == NotificationTargetProvided.DINGTALK.meta.uuid:
        if dingtalk_target_form is None:
            if require_config:
                raise schemas.error.CustomException(message="DingTalk target configuration is required", code=400)
            return None
    ...
    if target_provided_uuid == NotificationTargetProvided.TELEGRAM.meta.uuid:
        if telegram_target_form is None:
            if require_config:
                raise schemas.error.CustomException(message="Telegram target configuration is required", code=400)
            return None
        if not telegram_target_form.chat_id:
            raise schemas.error.CustomException(message="Chat ID is required", code=400)
        return json.dumps(
            {
...
async def notification_ios_target_change_code_status(
    ...
    if not delivered:
        info_logger.info(
            format_log_message(
                "notification_websocket_message_cached",
                user_id=user.id,
                websocket_id=f"web-{user.uuid}",
            )
        )
