...
- 体験版: <https://app.revornix.com>（中国本土からアクセスする場合は接続が安定しやすいミラー <https://app.revornix.cn> の利用を推奨）
...
- **音声文字起こし** —— 音声ドキュメントを検索可能な Markdown に変換。話者分離に対応した会議モードも提供。
...
- **通知システム** —— メール／アプリ内／プッシュなど複数チャネルから、イベント（処理完了・コメント・購読・共同編集リクエストなど）に応じて配信。
- **アカウントセキュリティ（MFA）** —— TOTP 認証アプリとパスキーに対応し、最後のログイン方法の削除を防止。
...
> [!NOTE]
> `app/`、`api/`、`worker/` は 1 つの **uv workspace** です。`uv.lock` は 1 つ、`.venv` はリポジトリ直下に 1 つだけ。ルートで一度インストールすればよく、サービスごとの環境分離は不要です。
...
> **初期化の手順はありません。** API は起動のたびにテーブル作成・カラム追加・
> 組み込みデータの投入を行い、Milvus のコレクションの存在も確認します。新規
> インストールもアップグレードも、サービスを起動するだけで済みます。README に
> 書かれた手動手順は、いずれ誰かが飛ばす手順です。
...
**この手順は省略できません。** 以前はここに「任意」と書かれていましたが、それは誤りでした。
`web/.env.example` の `NEXT_PUBLIC_API_PREFIX` が指すのは `http://localhost:8787/api`、
つまり api の 8001 ではなく**ゲートウェイ**です。「任意」だと思って飛ばすと、
フロントエンドのリクエストはすべて誰も待ち受けていないポートに送られ、画面は
"Load failed" だらけになります。それでいて api のログはまったくの空で、
異常を示す手がかりが何も残りません。

ゲートウェイは公開トラフィックの入口、上流フェイルオーバー、`api/` に到達する前の
アンチスクレイピング/レート制限を担当します。トレンド集約もここを経由します
（`8787/hot` → hot-news の 6688）。
